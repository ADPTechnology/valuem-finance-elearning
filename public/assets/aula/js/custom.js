...
$(function () {
    ...
    $("html").on("click", "#close-auth-user", function (e) {
        e.preventDefault();

        SwalExit.fire().then(function (e) {
            if (e.value === true) {
    ...
    $("html").on("click", "#return-main-view", function (e) {
        e.preventDefault();
        let button = $(this);

        console.log(button.attr("href"));

        SwalReturnMain.fire().then(function (e) {
            if (e.value === true) {
            ...
            $.ajax({
                ...
                success: function (data) {
                    avatarEditModal
                        .find(".img-holder")
                        .html(
                            '<img class="img-fluid avatar_img" id="image-user-avatar-edit" src="' +
                                data.url_img +
                                '"></img>'
                        );
                    avatarEditModal
                        .find("#input-user-avatar-edit")
                        .attr(
                            "data-value",
                            '<img scr="' +
                                data.url_img +
    ...
    if (jQuery.type(videoElement) != "undefined") {
        ...
        var sectionId = dataChapterInput.data("section");
        var productCertificationId = dataChapterInput.data('prodcert');

        var iconCheck = '<i class="fa-solid fa-circle-check"></i>';
        var iconNoCheck = '<i class="fa-regular fa-circle"></i>';

        var sectionContainer = $('#collapse-' + sectionId)
        var incompleteFlag = sectionContainer.find('#incomplete-flag-' + sectionId).length ? true : false
        ...
        function SetTimeProgress() {
            ...
            var data = {
                ...
                productCertificationId: productCertificationId
            };
            var headers = {
                "X-CSRF-TOKEN": $('meta[name="csrf-token"]').attr("content"),
            };
            ...
            $.ajax({
                ...
                success: function (response) {
                    if (response.check) {
                        ...
                        var btnStartContainer = sectionContainer.find('.btn-start-evaluation-container')

                        if (incompleteFlag && response.completedChapters) {
                            btnStartContainer.html(response.htmlBtn)
                        }
        ...
        videoElement.on("playing", () => {
            timeVideoUpdate = setInterval(SetTimeProgress, 3000);
