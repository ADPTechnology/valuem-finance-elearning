...
const companies = new CountUp("count-up-companys", n_companys, {
    // prefix: "+ ",
    // suffix: " 🏢",
    duration,
    separator: ",",
...
if (!students.error && !courses.error && !companies.error) {
    students.start();
    courses.start();
    companies.start();
} else {
    console.error(students.error, courses.error, companies.error);
}
