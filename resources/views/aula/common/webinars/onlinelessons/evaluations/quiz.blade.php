            ...
            <div class="card quiz z-index-2">
                ...
                @if (!is_null($exam))
                    <div class="info">
                        ...
                        <h3> {{ $certification->event->course->description }} </h3>
                    ...
                    <ul id="progressbar" class="width-progress" style="--progressWidth: {{ count($evaluations) }}">
                        ...
                        @foreach ($evaluations as $key => $eval)
                            @if ($selected_answers >= $key)
                                <a
                                    href="{{ route('aula.course.quiz.show', [$certification, 'num_question' => $key + 1]) }}">
                    ...
                    @if ($type_id == 1 || $type_id == 3)
                        @include('aula.viewParticipant.courses.evaluations.types.unique_answer', [
                            'route' => route('aula.freeCourseLive.evaluations.quiz.show', [
                                $certification,
                                $num_question,
                            ]),
                            'routeUpdate' => route('aula.freeCourseLive.evaluations.quiz.update', [
                                $certification,
                                $exam,
                                $num_question + 1,
                                $key + 1,
                    ...
                    @elseif ($type_id == 4)
                        @include('aula.viewParticipant.freeCourseLive.evaluations.types.fill_in_the_blank', [
                            'route' => route('aula.freeCourseLive.evaluations.quiz.show', [
                                $certification,
                                $num_question,
                            ]),
