@foreach ($course->courseSections as $section)
    <div class="course-section-box {{ setSectionActive($section, $sectionActive) }}"
        ...
        data-table="{{ route('admin.freeCourses.chapters.getDataTable', $section) }}" id="section-box-{{ $section->id }}"
        data-id="{{ $section->id }}">
        ...
        <div class="order-select-container">
            ...
            <div class="input-group">
                <select name="order" data-url="{{ route('admin.freeCourses.sections.updateOrder', $section) }}"
                    class="form-control select2 order-section-select">
                    @foreach ($course->courseSections as $order)
                        <option {{ getSelectedOption($section, $order->section_order) }}
                            value="{{ $order->section_order }}">
                            {{ $order->section_order }}
                        </option>
                    @endforeach
                </select>
        ...
        <div class="action-box clean-list">
            <div class="btn-action-container">
                <span class="section-edit-btn edit-btn"
                    data-send="{{ route('admin.freeCourses.sections.edit', $section) }}"
