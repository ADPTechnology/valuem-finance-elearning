...
class Certification extends Model
{
    ...
    public function fcSteps()
    {
        return $this->belongsToMany(FcStep::class, 'f_curve_step_progress', 'certification_id', 'f_curve_step_id')
                    ->withPivot(['id', 'status', 'score'])->withTimestamps();
    }

    public function fcStepProgress()
    {
        return $this->hasMany(FcStepProgress::class, 'certification_id');
    }
    ...
    {
        $messages = [];
        $today = getCarbonInstance(getCurrentDate());
        $event_date = getCarbonInstance($this->event->date);
        ...
        if ($today->lessThan($event_date)) array_push($messages, "Fuera de fecha.");
