...
class DynamicQuestionService
{
    ...
    // * DATA TABLE FREE COURSE EXAM

    public function getDataTableFreeCourse(int $exam_id = null)
    {
        $query = DynamicQuestion::with(['questionType:id,description'])
            ->withCount(['evaluations']);
        ...
        $allQuestions = DataTables::of($query)
            ->editColumn('statement', function ($question) {
                return '<a href="' . route('admin.freeCourses.exams.questions.show', $question) . '">' . $question->statement . '</a>';
            })
            ->editColumn('created_at', function ($question) {
                return $question->created_at;
            })
            ...
            ->addColumn('action', function ($question) {
                if ($question->evaluations_count == 0) {
                    $btn = '<a href="javascript:void(0)" data-id="' .
                        $question->id . '" data-original-title="delete"
                                        data-url="' . route('admin.freeCourses.exams.questions.destroy', $question) . '" class="ms-3 edit btn btn-danger btn-sm
                                        deleteQuestion-btn"><i class="fa-solid fa-trash-can"></i></a>';
                } else {
                    $btn = '<a href="javascript:void(0)" data-id="' .
                        $question->id . '" data-original-title="delete" class="ms-3 btn btn-danger disabled btn-sm"><i class="fa-solid fa-trash-can"></i></a>';
    ...
    public function getDataTableStep(int $exam_id = null)
    {
        $query = DynamicQuestion::with(['questionType:id,description'])
            ->withCount(['evaluations']);
        ...
        $allQuestions = DataTables::of($query)
            ->editColumn('statement', function ($question) {
                return '<a href="' . route('admin.forgettingCurve.steps.evaluation.questions.show', $question) . '">' . $question->statement . '</a>';
            })
            ->editColumn('created_at', function ($question) {
                return $question->created_at;
            })
            ...
            ->addColumn('action', function ($question) {
                if ($question->evaluations_count == 0) {
                    $btn = '<a href="javascript:void(0)" data-id="' .
                        $question->id . '" data-original-title="delete"
                                        data-url="' . route('admin.forgettingCurve.steps.evaluation.questions.destroy', $question) . '" class="ms-3 edit btn btn-danger btn-sm
                                        deleteQuestion-btn"><i class="fa-solid fa-trash-can"></i></a>';
                } else {
                    $btn = '<a href="javascript:void(0)" data-id="' .
                        $question->id . '" data-original-title="delete" class="ms-3 btn btn-danger disabled btn-sm"><i class="fa-solid fa-trash-can"></i></a>';
