...
class CertificationService
{
    ...
    public function externalSelfStore(User $user, WebinarEvent $event)
    {

        $event->loadCertificationsRelationships();
        $event->loadParticipantsCount();

        if ($event->room->capacity > $event->certifications_count) {
            $this->storeExternal($event, $user);
            return true;
        }
    ...
    public function updateAssist($request, Certification $certification)
    {
        ...
        return $certification->update($request->only('assist_user'));
    }
    ...
    private function storeExternal(WebinarEvent $event, User $user)
    {
        $event->certifications()->create([
            'user_id' => $user->id,
            'observation' => null,
            'unlock_cert' => 'N'
        ]);
    ...
    public function getInsParticipantsDatatable(User $user, Event $event)
    {
        $query = $event->certifications()->whereHas('event', function ($q) use ($user) {
        ...
        $allCertifications = DataTables::of($query)
            ...
            ->rawColumns(['status', 'assist_user', 'enabled', 'action'])
            ->make(true);
    ...
    public function getSupervisorParticipantsDataTable($request, Event $event)
    {
        $query = $event->certifications()
            ->where('evaluation_type', 'certification')
            ->with(['user.company', 'event'])
        ...
        $allCertifications = DataTables::of($query)
            ...
            ->addColumn('assit', function ($certification) {
                $assit_btn = '<label class="custom-switch">
                            <input type="checkbox" name="flg_asist" ' . $certification->event_assist_status . '
                                class="custom-switch-input flg_assist_user_checkbox disabled"
                                ' . $certification->valid_assist_checked . '>
                            <span class="custom-switch-indicator"></span>
                        </label>';
            ...
            ->addColumn('action', function ($certification) {
                $btn = '<button data-toggle="modal" data-id="' .
                    $certification->id . '" data-send="' . route('aula.supervisor.events.showCertification', $certification) . '"
                                    data-original-title="edit" class="me-3 edit btn btn-info btn-sm
                                    showCertification-btn"><i class="fa-solid fa-eye"></i></button>';
    ...
    public function requestRegistrationCourse(User $user, Course $course)
    {

        $course = $course->productCertifications()->create([
            'user_id' => $user->id,
            'status' => 'pending'
        ]);
    ...
    public function getByFilters($request)
    {
        $query = Certification::with(
            ...
            ->whereHas('event.user', function ($q5) {
                $q5->where('signature', 'S');
            })
            ->whereHas('user', function ($q6) {
                $q6->where('signature', 'S');
            })
            ->where('evaluation_type', 'certification')
            ->select('certifications.*');
    ...
    public function getWebinarParticipantsDataTable(User $user, WebinarEvent $event)
    {
        $query = $event->certifications()
        ...
        $allCertifications = DataTables::of($query)
            ->editColumn('unlock_cert', function ($certification) {

                $status = $certification->unlock_cert == 'N' ? 'Bloqueado' : 'Desbloqueado';

                $statusBtn = '<span class="status badge badge-fill badge-warning">' . $status . '</span>';
            ...
            ->rawColumns(['unlock_cert', 'enabled', 'action'])
            ->make(true);
