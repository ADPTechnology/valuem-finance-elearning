...
class AdminRoomsController extends Controller
{
    ...
    {
        $roomsExport = new RoomsReport;

        $rooms = Room::orderBy('id', 'desc')->limit(500)->get();

        $roomsExport->setRooms($rooms);

        $date_info = 'últimos_500';

        return $roomsExport->download(
            'reporte-salas_' . $date_info . '.xlsx'
        );
    }
