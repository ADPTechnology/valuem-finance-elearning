...
use App\Exports\OwnerCompaniesReport;
...
class AdminOwnerCompaniesController extends Controller
{
    ...
    {
        $ownerCompaniesExport = new OwnerCompaniesReport;

        $ownerCompanies = OwnerCompany::orderBy('id', 'desc')->limit(500)->get();

        $ownerCompaniesExport->setOwnerCompanies($ownerCompanies);

        $date_info = 'últimos_500';

        return $ownerCompaniesExport->download(
            'reporte-empresas-titulares_' . $date_info . '.xlsx'
        );
    }
